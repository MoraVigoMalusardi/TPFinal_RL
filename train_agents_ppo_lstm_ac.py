...
class CustomLSTMPostFC(RecurrentNetwork, nn.Module):
    def __init__(self, obs_space, action_space, num_outputs, model_config, name):
        ...
        # ------------------------------------------------------
        # 1. ENCODER (Una sola capa de reducción: 1276 -> 256)
        # ------------------------------------------------------
        ...
        # ------------------------------------------------------
        # 2. LSTM (Input 256 -> Hidden 128)
        # ------------------------------------------------------
        self.lstm = nn.LSTM(256, self.cell_size, batch_first=True)

        # ------------------------------------------------------
        # 3. RAMAS SEPARADAS (Post-LSTM)
        # La entrada a estas ramas es la salida de la LSTM (128)
        # ------------------------------------------------------

        # RAMA ACTOR: 2 capas de 256 + Lineal de salida (num_outputs)
        self.actor_branch = nn.Sequential(
            nn.Linear(self.cell_size, 256),
            nn.Tanh(),
            nn.Linear(256, 256),
            nn.Tanh(),
            nn.Linear(256, num_outputs)  # Salida lineal (Logits)
        )

        # RAMA CRITIC: 2 capas de 256 + Lineal de salida (1)
        self.critic_branch = nn.Sequential(
            nn.Linear(self.cell_size, 256),
            nn.Tanh(),
            nn.Linear(256, 256),
            nn.Tanh(),
            nn.Linear(256, 1)  # Salida lineal (Value)
        )
    ...
    def forward_rnn(self, inputs, state, seq_lens):        
        ...
        # B. Pasar por la LSTM
        # x: [B, T, 256] -> lstm_out: [B, T, 128]
        lstm_out, [h, c] = self.lstm(x, [torch.unsqueeze(state[0], 0), torch.unsqueeze(state[1], 0)])
        
        # C. Ramas Separadas
        # El Actor toma la salida de la memoria (128) y la procesa independientemente
        logits = self.actor_branch(lstm_out)
...
def create_tb_logger_creator(run_dir):
    ...
    def logger_creator(config):
        logdir = os.path.join(run_dir, "tb_logs_lstm_ac")
...
def train(trainer, num_iters=5):
    ...
    torch.save(
        ...
        "checkpoints/nuevo_con_lstm_ac/policy_a_weights.pt",
    )

    if "p" in trainer.workers.local_worker().policy_map:
        torch.save(
            trainer.get_policy("p").model.state_dict(),
            "checkpoints/nuevo_con_lstm_ac/policy_p_weights.pt",
        )

    # ==== Guardar checkpoint completo de RLlib ====
    checkpoint_root = os.path.join("checkpoints", "rllib_full_lstm_ac")
