...
logger = logging.getLogger("train_planner_lstm_ac")
...
def process_args():
    ...
    parser.add_argument(
        "--run-dir",
        type=str,
        default="phase_aiecon_lstm",
        help="Directorio que contiene config.yaml para Fase 2 (por defecto: phase_aiecon_lstm).",
    )
    parser.add_argument(
        "--num-iters",
        type=int,
...
# -------------------------------------------------------------------
# Custom LSTM + FC (MISMA ARQUITECTURA QUE EN FASE 1)
# -------------------------------------------------------------------

class CustomLSTMPostFC(RecurrentNetwork, nn.Module):
    """
    Arquitectura recurrente:
      obs -> Linear(…,256) -> Tanh -> LSTM(256 -> cell_size)
      -> rama actor (256,256) -> logits
      -> rama crítico (256,256) -> value
    """
    def __init__(self, obs_space, action_space, num_outputs, model_config, name):
        ...
        # 2) LSTM: 256 -> cell_size
        self.lstm = nn.LSTM(256, self.cell_size, batch_first=True)

        # 3) RAMA ACTOR: cell_size -> 256 -> 256 -> num_outputs
        self.actor_branch = nn.Sequential(
            nn.Linear(self.cell_size, 256),
            nn.Tanh(),
            nn.Linear(256, 256),
            nn.Tanh(),
        ...
        # 4) RAMA CRÍTICA: cell_size -> 256 -> 256 -> 1
    ...
    def forward_rnn(self, inputs, state, seq_lens):
        ...
        # Actor: logits por paso de tiempo
        logits = self.actor_branch(lstm_out)  # [B,T,num_outputs]
...
def build_multiagent_policies(env_obj, run_configuration):
    """
    Fase 2: entrenar principalmente el planner ("p"),
    usando agentes ("a") inicializados desde Fase 1.
    """
    ...
    logger.info("Agent & Planner usan CustomLSTMPostFC (LSTM + FC actor/critic)")
...
def build_trainer_config(env_obj, run_configuration, env_config):
    ...
    logger.info("Configuración del trainer PPO (Fase 2 LSTM+FC):")
...
def create_tb_logger_creator(run_dir):
    def logger_creator(config):
        logdir = os.path.join(run_dir, "tb_logs_planner_lstm_ac")
...
def train(trainer, num_iters=5, planner=True):
    ...
    if planner:
        torch.save(
            ...
            "checkpoints/nuevo_con_lstm_ac/policy_a_weights_w_planner.pt",
        )
        if "p" in trainer.workers.local_worker().policy_map:
            torch.save(
                trainer.get_policy("p").model.state_dict(),
                "checkpoints/nuevo_con_lstm_ac/policy_p_weights_w_planner.pt",
            )
    else:
        torch.save(
            trainer.get_policy("a").model.state_dict(),
    ...
    checkpoint_root = os.path.join("checkpoints", "rllib_planner_full_lstm_ac")
...
def run_eval_episode(trainer, env_obj, max_steps=200):
    """
    Eval con estados LSTM explícitos.
    """
    obs = env_obj.reset()
    done = {"__all__": False}
    ...
    for agent_id in obs.keys():
        ...
        lstm_states[(agent_id, policy_id)] = trainer.get_policy(policy_id).get_initial_state()
...
def main(planner=True):
    ...
    logger.info("=" * 70)
    logger.info("Iniciando entrenamiento FASE 2 (PLANNER LSTM+FC) con AI-Economist")
        ...
        if restore_agents_path and os.path.exists(restore_agents_path):
            ...
            try:
                ...
                if train_planner_flag:
                    logger.info("Pesos de agentes utilizados como base para Fase 2.")
