...
def run_eval_episode(trainer, env_obj, max_steps=200):
    ...
    manejando correctamente el estado de la LSTM.
    """
    obs = env_obj.reset()
    done = {"__all__": False}
    total_rewards = {agent_id: 0.0 for agent_id in obs.keys()}

    # Estados iniciales de la RNN por agente y política
    policy_a = trainer.get_policy("a")
    policy_p = trainer.get_policy("p") if "p" in trainer.workers.local_worker().policy_map else None

    rnn_states = {}
    for agent_id in obs.keys():
        policy_id = "a" if str(agent_id).isdigit() else "p"
        if policy_id == "a":
            rnn_states[agent_id] = policy_a.get_initial_state()
        else:
            rnn_states[agent_id] = policy_p.get_initial_state() if policy_p is not None else []
    ...
    while not done["__all__"] and step < max_steps:
        ...
        for agent_id, ob in obs.items():
            ...
            # full_fetch=True para obtener también el nuevo estado recurrente
            ...
            new_rnn_states[agent_id] = state_out
