...
def create_tb_logger_creator(run_dir):
    ...
    dentro de run_dir/tb_logs_planner_lstm_fc.
    """
    def logger_creator(config):
        logdir = os.path.join(run_dir, "tb_logs_planner_lstm_fc")
...
def train(trainer, num_iters=5, planner=True):
    ...
    os.makedirs("checkpoints/nuevo_con_lstm_fc", exist_ok=True)

    if planner:
        torch.save(
            trainer.get_policy("a").model.state_dict(),
            "checkpoints/nuevo_con_lstm_fc/policy_a_weights_w_planner.pt",
        )
        if "p" in trainer.workers.local_worker().policy_map:
            torch.save(
                trainer.get_policy("p").model.state_dict(),
                "checkpoints/nuevo_con_lstm_fc/policy_p_weights_w_planner.pt",
            )
    else:
        torch.save(
            trainer.get_policy("a").model.state_dict(),
    ...
    checkpoint_root = os.path.join("checkpoints", "rllib_planner_full_lstm_fc")
...
def main(planner=True):
    ...
    # Guardar historial
    os.makedirs("nuevo_con_lstm_fc", exist_ok=True)
    csv_path = "nuevo_con_lstm_fc/ppo_results_with_planner.csv"
